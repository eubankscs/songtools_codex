...
import './style.css';
import { parseBlocksJson, stringifyBlocks, toRenderableBlocks } from './editorModel';
import type { ChordPlacement, EditorBlock, EditorDocument, HomeSnapshot, ProjectDetail, ProjectRow, RecentSongRow, SongRow } from './types';
...
function ProjectView({ projectId, run, setView }: { projectId: string; run: (action: () => Promise<void>) => Promise<void>; setView: (view: View) => void }) {
  ...
  return (
    <section className="screen">
      ...
      <button className="secondary-action" onClick={() => void run(async () => { const title = window.prompt('Song title'); if (title) { const song = await window.songtools.createSong(title, projectId); setView({ name: 'song', songId: song.id }); } })}>+ New Song</button>
...
function SongPlaceholder({ songId, setView }: { songId: string; setView: (view: View) => void }) {
  ...
  async function saveBlocks() {
    ...
    const saved = await window.songtools.saveEditorBlocks(songId, blocks);
    setDocument(saved);
    setDraftJson(stringifyBlocks(saved.blocks));
    setMessage('Saved and reloaded exact chord offsets.');
  }
  ...
  return (
    <section className="editor-screen">
      <header className="editor-header">
        ...
        <h1>{document?.song.title ?? 'Song Editor'}</h1>
      </header>
      <button className="back" onClick={() => setView({ name: 'home' })}>← Home</button>
      {message && <p className="hint">{message}</p>}
      <EditorCanvas blocks={document?.blocks ?? []} />
      <section className="editor-data-panel" aria-label="Editor block data">
        ...
        <textarea value={draftJson} onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => setDraftJson(event.currentTarget.value)} aria-label="Editor blocks JSON" autoFocus />
        <button className="secondary-action" onClick={() => void saveBlocks().catch((problem: Error) => setMessage(problem.message))}>Save and Reload</button>
      </section>
    </section>
  );
}

function EditorCanvas({ blocks }: { blocks: EditorBlock[] }) {
  const renderBlocks = toRenderableBlocks(blocks);
  return (
    <section className="editor-canvas" aria-label="Editor canvas">
      ...
      {renderBlocks.map((block) => block.kind === 'section'
        ? <div key={`${block.kind}-${block.position}`} className={`section-tag ${block.isFirstSection ? 'first-section' : ''}`}>{block.text}</div>
        : <LyricWithChords key={`${block.kind}-${block.position}`} text={block.text} chords={block.chords} />)}
    </section>
  );
}

function LyricWithChords({ text, chords }: { text: string; chords: ChordPlacement[] }) {
  const columnCount = Math.max(text.length + 1, ...chords.map((placement) => placement.offset + placement.chord.length + 1), 1);
  return (
