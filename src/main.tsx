...
import './style.css';
import type { HomeSnapshot, ProjectDetail, ProjectRow, RecentSongRow, SongRow } from './types';
...
function SongPlaceholder({ songId, setView }: { songId: string; setView: (view: View) => void }) {
  return <section className="screen"><button className="back" onClick={() => setView({ name: 'home' })}>← Home</button><h1>Song Editor</h1><p className="empty">Editor implementation starts in Phase 3. Opened song ID: {songId}</p></section>;
}
